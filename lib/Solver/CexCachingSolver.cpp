...
class CexCachingSolver : public SolverImpl {
  ...
  bool guessSplit(const std::set<ref<Expr> > &parentKey,
						const ref<Expr> & newExpr,
						const Assignment * const &oldAnswer,
						Assignment *&result);

  /*
   * The optional skipStats flag is for calls generated by guessIndependent, which
   */
  bool getAssignment(const Query& query, Assignment *&result, bool skipStats = false);
...
bool CexCachingSolver::checkPreviousSolution(const Query &query,
	...
	/*
	 * Step 1: See if can short circuit using previous answer
	 *
	 * Note: may have to add functionality in case query.expr has pieces
	 * that previous answer does not (overwrite query.expr with query.constraints)
	 */
	std::set<ref<Expr> > parentKey;
	ref<Expr> queryExpr;
	if(isa<ConstantExpr>(query.expr)){
		assert(cast<ConstantExpr>(query.expr)->isFalse() && "query.expr == true shouldn't happen");
	...
	if(checkPreviousSolutionHelper(queryExpr, parentKey, parentSolution, result)){
		...
		/*
		 * While normally !assignment would mean that the answer is UNSAT, since
		 * checkPreviousSolution helper returned false, this means we were unable
		 * to find a solution to the previous state, and therefore don't have anything
		 * to build off of.
		 */
		return false;
	}else{
		return guessSplit(parentKey, queryExpr, parentSolution, result);
	}
}

bool CexCachingSolver::guessSplit(const std::set<ref<Expr> > &parentKey,
								  const ref<Expr> & queryExpr,
								  const Assignment * const &oldAssignment,
								  Assignment * &result){
	...
	IndependentElementSet ies = getFreshFactorUnsafe(parentKey, queryExpr, unsafeFactor);

	if(unsafeFactor.size() >= parentKey.size() || ies.elements.size() == 0){
		return false;
	}

	Assignment * newestAssignment = 0;
	ConstraintManager tmp(unsafeFactor);
	Query query = Query(tmp, queryExpr);
	 ...
	 */
	if(! getAssignment(query, newestAssignment, true)){
		return false;
	}
	...
	if(! newestAssignment){
		...
		assert(!result && "the incoming result should be 0 so no need to change it");
		return true;
	}

	Assignment * possibleSolution = new Assignment(* oldAssignment, * newestAssignment, ies);
	if(queryExprEvaluatesToTrue(possibleSolution, queryExpr)  && parentKeyEvaluatesToTrue(possibleSolution, parentKey)){
